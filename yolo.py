...
class YOLO:
    ...
    def __init__(self, config, model, labels, size=256, confidence=0.3, threshold=0.3):
    ...
    def infer_bbox(self, image):
        iw, ih, inference_time, results = self.inference(image)
        num_res = len(results)
        if num_res == 0:
            return None
        else:
    ...
    def bbox_from_detect(self, results):
        bbox = []
        if len(results) > 0:
            for detection in results:
